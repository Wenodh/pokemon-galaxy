...
const categoryColors: Record<string, string> = { physical: "text-red-500", special: "text-blue-500", status: "text-zinc-500", };
      ...
      <div className="mb-8 flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><input type="text" placeholder="Search moves..." value={search} onChange={(e) => setSearch(e.target.value)} className="h-10 w-full rounded-full border border-border/50 bg-background/50 pl-10 pr-4 text-sm focus:border-primary/50 focus:outline-none focus:ring-1 focus:ring-primary/50 md:w-64" />
          </div>
          ...
          <tbody className="divide-y divide-border/30">
            {filteredMoves.map((move) => (
              <tr key={move.name} className="group hover:bg-primary/5 transition-colors">
                ...
                <td className="py-4"><Badge className={cn("rounded-sm px-2 py-0.5 text-[10px] font-black uppercase tracking-wider text-white", typeColors[move.type.toLowerCase()] || "bg-slate-500")}>{move.type}</Badge></td>
