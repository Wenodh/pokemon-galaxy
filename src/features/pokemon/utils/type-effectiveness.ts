...
export function calculateTypeEffectiveness(defendingTypes: string[]): TypeEffectiveness {
  ...
  const result: TypeEffectiveness = { weaknesses: [], resistances: [], immunities: [], };
  Object.entries(effectiveness).forEach(([type, multiplier]) => {
    if (multiplier > 1) { result.weaknesses.push({ type, multiplier }); } else if (multiplier > 0 && multiplier < 1) { result.resistances.push({ type, multiplier }); } else if (multiplier === 0) { result.immunities.push({ type, multiplier }); }
  });
